...
class VoiceIDPlugin(Plugin):
    ...
    def setup(self, ctx: PluginContext):
        ...
        # ToolHandler builds tools as cls(handler), so we cant pass the
        # recognizer through __init__. Stash on the class instead.
        VoiceIDTools._recognizer = recognizer
        ctx.register_tool(VoiceIDTools)

        ctx.subscribe("mic_chunk", recognizer.feed_audio)
    ...
    def teardown(self, ctx: PluginContext):
        # nothing to clean up, saves happen inline. flushing again here is fine.
        try:
